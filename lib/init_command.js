...
module.exports = class Command {
  ...
  * run(cwd, args) {
    ...
    const mapGenerates = [ 'controller', 'co', 'service', 's', 'middleware', 'mi', 'schedule', 'sd', 'extend', 'ex' ];
    if (mapGenerates.some(el => argv[el])) {
      this.generate();
      return;
    }
    // console.log('%j', argv);
  ...
  * processFiles(targetDir, templateDir) {
  ...
  getParserOptions() {
    return {
      ...
      controller: {
        type: 'boolean',
        description: 'generate egg controller',
        alias: 'co',
      },
      middleware: {
        type: 'boolean',
        description: 'generate egg middleware',
        alias: 'mi',
      },
      schedule: {
        type: 'boolean',
        description: 'generate egg schedule',
        alias: 'sd',
      },
      extend: {
        type: 'boolean',
      ...
      service: {
        type: 'boolean',
        description: 'generate egg service',
        alias: 's',
      },
    };
  }
  ...
  * getPackageInfo(pkgName, withFallback) {
  ...
  generate() {
    // default's architecture is controller
    let architecture = 'controller';

    // generate service
    if (this.argv.service || this.argv.s) {
      architecture = 'service';
    }

    // generate middleware
    if (this.argv.middleware || this.argv.mi) {
      architecture = 'middleware';
    }

    // generate schedule
    if (this.argv.schedule || this.argv.sd) {
      architecture = 'schedule';
    }

    // generate extend
    if (this.argv.extend || this.argv.ex) {
      architecture = 'extend';
    }

    // check dir exists
    const architecturePath = path.join(this.cwd, `app/${architecture}/`);
    if (!fs.existsSync(architecturePath)) {
      mkdirp.sync(architecturePath);
    }

    const store = memFs.create();
    const fsEditor = editor.create(store);
    const fileName = this.argv._ || [ 'index' ];
    const createFile = _fileName => {
      let FileName = 'Index';
      if (_fileName) {
        FileName = _fileName.replace(/(\w)/, v => v.toUpperCase());
      }
      const tmpFilePath = path.join(__dirname, `/template/${architecture}/index.tpl`);
      const tmpFileText = fsEditor.read(tmpFilePath).replace(/<%= name %>/g, FileName);

      const toFilePath = path.join(architecturePath, `${_fileName}.js`);
      this.log(toFilePath);
      fs.writeFileSync(toFilePath, tmpFileText);
      this.log(`generate ${architecture} file : ${toFilePath}`);
    };
    for (const _fileName of fileName) {
